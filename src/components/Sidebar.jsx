import { useState } from 'react'
import SidebarItem from "./SidebarItem";
import { data } from "../assets/MenuData";
import { RiArrowDropRightLine } from "react-icons/ri";

const Sidebar = () => {

    const [isOpen, setIsOpen] = useState(true)

    return (
    <div className="sidebar" style={{width: isOpen ? "308px" : "70px"}}>
      <div className="logo" onClick={() => setIsOpen(!isOpen)} >
        <svg
          width="37"
          height="37"
          viewBox="0 0 37 37"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
        >
          <path
            d="M18.5002 34.8875C17.4673 34.8875 16.419 34.6562 15.6019 34.1783L7.12278 29.2912C3.66945 26.9633 3.45361 26.6087 3.45361 22.955V14.0442C3.45361 10.3904 3.65403 10.0358 7.0457 7.73874L15.5865 2.80541C17.2052 1.86499 19.749 1.86499 21.3677 2.80541L29.8777 7.70791C33.3311 10.0358 33.5469 10.3904 33.5469 14.0442V22.9396C33.5469 26.5933 33.3465 26.9479 29.9548 29.245L21.414 34.1783C20.5815 34.6562 19.5331 34.8875 18.5002 34.8875ZM18.5002 4.42416C17.8527 4.42416 17.2207 4.54749 16.7736 4.80958L8.29445 9.71208C5.78153 11.4079 5.78153 11.4079 5.78153 14.0442V22.9396C5.78153 25.5758 5.78153 25.5758 8.35611 27.3179L16.7736 32.1742C17.6832 32.6983 19.3327 32.6983 20.2423 32.1742L28.7215 27.2717C31.219 25.5758 31.219 25.5758 31.219 22.9396V14.0442C31.219 11.4079 31.219 11.4079 28.6444 9.66583L20.2269 4.80958C19.7798 4.54749 19.1477 4.42416 18.5002 4.42416Z"
            fill="black"
          />
          <path
            d="M18.5 24.2812C15.3088 24.2812 12.7188 21.6912 12.7188 18.5C12.7188 15.3088 15.3088 12.7188 18.5 12.7188C21.6912 12.7188 24.2812 15.3088 24.2812 18.5C24.2812 21.6912 21.6912 24.2812 18.5 24.2812ZM18.5 15.0312C16.5883 15.0312 15.0312 16.5883 15.0312 18.5C15.0312 20.4117 16.5883 21.9687 18.5 21.9687C20.4117 21.9687 21.9687 20.4117 21.9687 18.5C21.9687 16.5883 20.4117 15.0312 18.5 15.0312Z"
            fill="black"
          />
        </svg>
        <span style={{display: isOpen ? "block" : "none"}}>Dashboard</span>
      </div>
      {data.map((item, index) => (
        <SidebarItem key={index} item={item} />
      ))}
      <div className='time' style={{display: isOpen ? "block" : "none"}}>
          <span>14 : 38 : 15</span>
      </div>
      <div className='sidebar-bottom'>
      <img src='data:image/png;base64,' />
        <div>
          <span className='name'>Turdiyev Sheroz</span>
          <span className='degree'>Project Manager</span>
        </div>
        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M6 9L12 15L18 9" stroke="#757575" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
        </svg>

      </div>

    </div>
  );
};

export default Sidebar;

